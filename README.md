...
Разработанное приложение повторяет основной функционал известного приложения Instagram. 
...
Если у пользователя нет аккаунта, то он имеет возможность зарегестрироваться, заполнив необходимые поля формы регистрации.
...
Так же при публикации нового поста, пользователь может упомянуть пользователя указав его имя в формате **@username**, тогда упомянутый пользователь получит соответствующее уведомление об этом на экране уведомлений. Так же при публикации поста, можно указать **хэштег** для него.
...
По умолчанию приложение отображает страницу на которой отображаются все опубликованные изображения всеми пользователями.

 <p align="center"> 
<img width="318" height="551" src="./ReadmeResourses/Search.png">
</p>

Если нажать на строку поиска, то отобразится экран поиска по всем пользотвателям зарегестрированным в приложении.

 <p align="center"> 
<img width="318" height="551" src="./ReadmeResourses/UserSearch.png">
</p>
...
Пользователь выбирает изображение из его фотогалереи. Затем добавляет к нему описание и публикует.
 ...
 <p align="center"> 
<img width="318" height="551" src="./ReadmeResourses/UploadPhoto.png">
</p>

##  Центр уведомлений

Если пользователь зашел в приложение и у него есть непрочитанные уведомлений то об этом оповестит соответствующий индикатор.

 <p align="center"> 
<img width="318" height="551" src="./ReadmeResourses/NotificationPoint.png">
</p>

Как можно заметить уведомления бывают разных типов. Если пользователь на нас подписался, то рядом с уведомлением будет кнопка подписаться на этого пользователя, а если мы на него уже подписаны, то будет кнопка отписаться. Так же присутствуют уведомления об упоминании пользователя в комментариях и публикациях и о том, что кто-то лайкнул какую-либо его публикацию.
...
В приложении есть возможность переписываться с пользователями которые подписаны на текущего пользователя. Сообщения сортируются по новизне. Чтобы написать пользователю нужно выбрать соответствующий диалог или нажать **+** и выбрать пользователя чтобы начать диалог.
